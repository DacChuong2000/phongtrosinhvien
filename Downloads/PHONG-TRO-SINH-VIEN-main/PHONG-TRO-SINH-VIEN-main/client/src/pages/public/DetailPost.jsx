...
const DetailPost = ({ navigate, location }) => {
  ...
  const fetLngLat = async (payload) => {
    ...
    if (response.status === 200)
      setCenter([response.data?.features[0]?.properties?.lat, response.data?.features[0]?.properties?.lon])
  }
  useEffect(() => {
    if (post?.address) {
      fetLngLat({
          ...
          <div className="p-4 text-sm flex flex-col gap-3">
            ...
            <div className="mt-6">
              ...
              <div className="w-full h-[250px]">
                {center.length > 0 && <Map center={center} address={post?.address} zoom={16} />}
                {center.length === 0 && (
